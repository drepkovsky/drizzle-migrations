...
const VALID_MIGRATION_EXTENSIONS = new Set([
  '.ts',
  '.js',
  '.cjs',
  '.mjs',
...
export async function getMigrationFiles(ctx: MigrationContext) {
  ...
  if (!fs.existsSync(dir)) {
    throw new Error(
      `Migration folder ${dir} does not exist. Please create it first.`,
    );
  }
...
export async function ensureMigrationTable(ctx: MigrationContext) {
  ...
  if (ctx.dialect === 'postgresql') {
    ...
    await ctx.client.execute(
      sql.raw(`CREATE SCHEMA IF NOT EXISTS ${migrationSchema}`),
    );
...
export function getLatestBatch(ctx: MigrationContext) {
  ...
  if (ctx.dialect === 'mysql') {
    return ctx.client
      .execute(
        sql.raw(`SELECT MAX(batch) as batch FROM "${ctx.migrationTable}"`),
      )
  ...
  if (ctx.dialect === 'postgresql') {
    return ctx.client
      .execute(
        sql.raw(
          `SELECT MAX(batch) as batch FROM "${ctx.migrationSchema}"."${ctx.migrationTable}"`,
        ),
      )
      .then(r => {
...
export function getMigrationBatch(
  ...
  if (ctx.dialect === 'sqlite') {
    return ctx.client
      .run(
        sql.raw(
          `SELECT batch FROM "${ctx.migrationTable}" WHERE name = '${migrationName}'`,
        ),
      )
      .then((r: any) => r[0]?.batch as number);
  }
...
export function deleteMigrationByName(
  migrationName: string,
  ctx: MigrationContext,
) {
  if (ctx.dialect === 'sqlite') {
    return ctx.client.run(
      sql.raw(
        `DELETE FROM "${ctx.migrationTable}" WHERE name = '${migrationName}'`,
      ),
    );
  }
  if (ctx.dialect === 'mysql') {
...
export function deleteMigrationUntilBatch(
  batch: number,
  ctx: MigrationContext,
) {
  if (ctx.dialect === 'sqlite') {
